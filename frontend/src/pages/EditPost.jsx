...
const EditPost = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!title || !content) {
      setError('Title and Content are required to update the signal.');
      return;
    }
    ...
    try {
      ...
      setError('Failed to update signal. ' + (err.response?.data?.message || err.message));
      setSubmitting(false);
    }
  };

  if (loading) return <div className="container fade-in">Decrypting data for editing...</div>;

  return (
    <div className="container fade-in delay-1">
      <Link to={`/posts/${id}`} style={{display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '2rem', color: 'var(--text-secondary)'}}>
        <ArrowLeft size={18} />
        Back to Signal
      </Link>

      <div className="glass-panel" style={{padding: '2.5rem', maxWidth: '800px', margin: '0 auto'}}>
        <h1 style={{marginBottom: '2rem'}}>Reconfigure Signal</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="btn btn-primary" disabled={submitting} style={{width: '100%', padding: '1rem'}}>
            <Save size={18} />
            {submitting ? 'Applying reconfiguration...' : 'Apply Reconfiguration'}
          </button>
        </form>
      </div>
    </div>
